...
class CustomDataset(Dataset):
    def __init__(self, data_name, file_path, time_windows, step, mode='Train'):
        ...
        if data_name == 'Yahoo':
            ...
            labels = dataframe['is_anomaly'].values
            # 提取正常数据的最大值最小值
            normal_data = dataframe[dataframe['is_anomaly'] == 0].astype(float)
            normal_data = normal_data['value'].values
        elif data_name == 'KPI':
            data_dim = 1
            dataframe = pd.HDFStore(file_path).get('data')
            dataframe = dataframe[:10000:2]
            values = dataframe['value'].values
            labels = dataframe['label'].values

            normal_data = dataframe[dataframe['label'] == 0]
            normal_data = normal_data['value'].values
        elif data_name == 'SMAP':
            data_dim = 25
            values = pickle.load(open('../data/NASA/SMAP/SMAP_test.pkl', 'rb'), encoding='utf-8')
            normal_data = values.copy()
            labels = pickle.load(open('../data/NASA/SMAP/SMAP_test_label.pkl', 'rb'), encoding='utf-8')
            mode = 'Test'
        elif data_name == 'MSL':
            data_dim = 55
            values = pickle.load(open('data/NASA/MSL/MSL_test.pkl', 'rb'), encoding='utf-8')
            normal_data = values.copy()
        ...
        # 用正常数据的最大值最小值做归一化
        normalized_data = scaler.transform(values.reshape(-1, data_dim))
        ...
        for i in range(0, (self.data_length - time_windows) // step + 1, step):
            ...
            if mode == 'Train':  # 用正常数据训练
                if sum(labels[i:i + time_windows]) == 0:
...
class PredictorDataset(Dataset):
    def __init__(self, data_name, file_path, time_windows, step, mode='Train'):
        ...
        if data_name == 'Yahoo':
            ...
            data_dim = 25
            mode = 'Test'  # SMAP和MSL不需要归一化，且有单独的训练集测试集
            values = pickle.load(open(file_path, 'rb'), encoding='utf-8')
            normal_data = values.copy()
            labels = []
        elif data_name == 'MSL':
            data_dim = 55
