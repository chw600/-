...
# label_index = np.array([i for i in range(train_mask.shape[0]) if not np.all(y_val[i]==0)])
# mask_index = np.argwhere(val_mask==1).flatten()
# print((label_index == mask_index).all())
# D^-1@X
